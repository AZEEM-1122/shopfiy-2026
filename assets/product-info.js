if (!customElements.get('product-info')) {
  customElements.define(
   ...
   // 1. Get the name of the option that was just changed
const changedOption = this.getAttribute('data-option-name'); // Ensure your HTML has this, or use index

// 2. Only swap the gallery if the change was a COLOR change
if (changedOption === 'Color' || changedOption === 'Colour') {
  if (this.querySelector('media-gallery').getAttribute('has_variant') === 'true') {
    ...
    // This replaces the images so you only see Grey images
    mediaGallerySource.outerHTML = mediaGalleryDestination.outerHTML;
    return;
  }
} else {
  // 3. If they clicked "Size" (2-seater/3-seater), DON'T swap the gallery.
  // Just use the default Dawn logic to scroll to the featured image.
  this.renderProductInfo(); 
}
